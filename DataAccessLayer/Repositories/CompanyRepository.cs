...
namespace DataAccessLayer.Repositories
{
    public class CompanyRepository : ICompanyRepository
    {
        ...
        {
            var companylist = await _companyDbWrapper.FindAsync(t => t.CompanyCode.Equals(companyCode));
            return companylist.FirstOrDefault();
        }
        public async Task<Company> GetCompanyByName(string companyName)
        {
            var companylist = await _companyDbWrapper.FindAsync(t => t.CompanyName.Equals(companyName));
            return companylist.FirstOrDefault();
        }

        public async Task<bool> Post(Company company)
        {
            return await _companyDbWrapper.InsertAsync(company);
        }

        public async Task<bool> Put(Company company)
        {
